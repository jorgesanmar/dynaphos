...
from dynaphos.safety.io import (  # noqa: E402
    DEFAULT_OUTPUT_ROOT,
    DEFAULT_SAFETY,
...
from dynaphos.safety import single_case  # noqa: E402


DEFAULT_INPUT_ROOT = Path(DEFAULT_OUTPUT_ROOT)
DEFAULT_OUTPUT_ROOT = Path(DEFAULT_VISUALS_ROOT)
...
def factors_from_manifest_and_data(
    ...
    preprocessing_label = preprocessing_key
...
def xy_from_coords_yaml(
    ...
    if manifest is None:
        return None
    coords_value = manifest.get("coords_yaml")
    if not coords_value:
        return None
    try:
        coords_path = resolve_repo_path(coords_value)
        coords = load_yaml(coords_path)
        x_all = np.asarray(coords.get("x", []), dtype=np.float64).reshape(-1)
        y_all = np.asarray(coords.get("y", []), dtype=np.float64).reshape(-1)
    except (OSError, TypeError, ValueError):
        return None
    if x_all.size != y_all.size or x_all.size == 0:
        return None

    electrode_ids = get_data_array(data, "electrode_ids", dtype=np.int64)
    if electrode_ids is not None:
        ids = np.asarray(electrode_ids, dtype=np.int64).reshape(-1)
        if ids.size == n_electrodes and ids.size > 0 and ids.min() >= 0 and ids.max() < x_all.size:
            return np.column_stack([x_all[ids], y_all[ids]])

    if x_all.size == n_electrodes:
...
def electrode_xy_for_charge_map(
    ...
    xy_yaml = xy_from_coords_yaml(data, manifest, n_electrodes)
    if xy_yaml is not None:
        return xy_yaml
...
def bioheat_layers_from_manifest(manifest: Mapping[str, Any]) -> list[tuple[float, float, str]]:
    params_value = manifest.get("params")
    if not params_value:
        return []
    try:
        params = load_yaml(resolve_repo_path(params_value))
    except (OSError, TypeError, ValueError):
        return []
    bioheat = params.get("bioheat", {}) or {}
    try:
        brain_depth = float(bioheat.get("brain_depth_mm", math.nan))
        skull = float(bioheat.get("skull_thickness_mm", 0.0))
        scalp = float(bioheat.get("scalp_thickness_mm", 0.0))
    except (TypeError, ValueError):
        return []
    if not np.isfinite(brain_depth) or brain_depth <= 0.0:
        return []
    layers = [(0.0, brain_depth, "brain")]
    if np.isfinite(skull) and skull > 0.0:
        layers.append((brain_depth, brain_depth + skull, "skull"))
    if np.isfinite(scalp) and scalp > 0.0:
        start = brain_depth + max(skull, 0.0)
        layers.append((start, start + scalp, "scalp"))
    return layers
...
def plot_matrix_summary_grid(
    ...
    *,
    ...
    for idx, (ax, fixed_value, values) in enumerate(zip(axes.reshape(-1), fixed_values, grids)):
        ...
        ax.set_xticks(range(len(x_values)))
        ax.set_xticklabels([value.label for value in x_values], rotation=30, ha="right", fontsize=8)
        ax.set_yticks(range(len(y_values)))
        ax.set_yticklabels([value.label for value in y_values], fontsize=8)
        ax.set_xlabel(factor_display_name(x_factor))
        if idx == 0:
