...
def condition_label(record: IcPowerRecord) -> str:
    return f"{record.grid}, {record.preprocessing}, {record.amplitude_uA:g} uA"
...
def plot_temperature_evolution_by_preprocessing(
    ...
    *,
    ...
    for grid, preprocessing, amplitude_uA in sorted(
        ...
        fig.suptitle(f"Temperature evolution - {grid}, {preprocessing}, {amplitude_uA:g} uA")
        if not plotted:
            for ax in axes:
...
def plot_temperature_value_grid(
    ...
    *,
    ...
    ax.set_ylabel("video-driven condition")
    ax.set_title(title)
    ...
    *,
    ...
    for (grid, preprocessing, amplitude_uA), condition_records in records_by_condition(records).items():
        ...
        for ax in axes:
            ...
            if ax.get_legend_handles_labels()[0]:
                ax.legend(title="IC power", frameon=False, fontsize=8, title_fontsize=8)
        fig.suptitle(f"Electrode Joule-heating offset - {grid}, {preprocessing}, {amplitude_uA:g} uA")
        if not plotted:
            for ax in axes:
    ...
    *,
    ...
    for result in results:
        ...
        ax.plot(
            ...
            result.fitted_dT_C,
            color="#8B1E3F",
            label=f"origin fit: {result.slope_C_per_mW:.6g} C/mW",
        )
...
def write_ic_power_temperature_visuals(
    ...
    *,
    ...
    if not phase1_records:
        raise RuntimeError(f"No phase 1 safety_metrics.npz files found under: {resolve_repo_path(phase1_input_root)}")
