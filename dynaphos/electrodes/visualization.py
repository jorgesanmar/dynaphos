...
def resolve_default_grid() -> Path:
    return package_file("coords_800um")


def resolve_default_grids() -> list[Path]:
    return [
        package_file("coords_400um"),
        package_file("coords_800um"),
        package_file("coords_1200um"),
    ]
...
def get_phosphene_coordinates(
    ...
    phosphene_map, _ = cortex_models.get_visual_field_coordinates_from_cortex_full(
        params["cortex_model"],
        coordinates_cortex,
        rng=rng,
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "One or more coordinate YAML files. Defaults to the packaged "
            "400 um, 800 um, and 1200 um arrays."
        ),
    )
    ...
    parser.add_argument(
        "--out",
        type=str,
        default="results/electrode_grid/visuals",
