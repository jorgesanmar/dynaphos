...
def load_safety_run(npz_path: Path, manifest: dict) -> SafetyRun:
    ...
    with np.load(npz_path, allow_pickle=True) as data:
        ...
        video_end_s = (
            float(np.asarray(data["cooldown_start_s"]).reshape(-1)[0])
            if "cooldown_start_s" in data.files
            else duration_s
        )
...
def plot_worst_case_temperature_evolution(
    ...
    *,
    ...
    if np.isfinite(baseline.video_end_s):
        ax.axvline(
            baseline.video_end_s / 60.0,
            color="#4B5563",
            linestyle="--",
            linewidth=1.1,
            alpha=0.85,
...
def run_phase3_analysis(
    ...
    *,
    ...
    phase2_summary = (
        resolve_repo_path(phase2_root)
        / "comparative_visuals"
        / "ic_power_linearity_summary.csv"
    )
    return write_raster_effect_outputs(
        root,
        phase1_root,
        output,
        ic_power_linearity_summary=phase2_summary,
        image_format=image_format,
        overwrite=overwrite,
    )
