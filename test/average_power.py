...
def run_video_tracking():
    ...
    output_folder_name = "average_power_tracking" # Changed output folder
    ...
    # --- 4. CALCULATE DUTY CYCLE SCALING ---
    # Formula: P_avg = P_peak * (2 * PW * Freq) * Rel_Stim_Duration
    stim_params = params['default_stim']
    pw = float(stim_params['pw_default'])          # Pulse Width (seconds)
    freq = float(stim_params['freq_default'])      # Frequency (Hz)
    rel_stim = float(stim_params['relative_stim_duration']) #
    
    # Factor 2.0 comes from biphasic pulses (hardcoded in simulator.py)
    duty_cycle_scaling = 2.0 * pw * freq * rel_stim
    
    print("-" * 40)
    print(f"CALCULATING AVERAGE POWER")
    print(f"Pulse Width: {pw*1e6:.1f} µs")
    print(f"Frequency:   {freq:.1f} Hz")
    print(f"Rel Duration:{rel_stim:.2f}")
    print(f"Scaling Factor (Duty Cycle): {duty_cycle_scaling:.6f}")
    print("-" * 40)
    ...
    try:
        while True:
            ...
            # --- POWER CALCULATION ---
            state = sim.get_state()
            # 1. Get Instant Peak Power (Watts)
            peak_power_tensor = state['instant_power']
            
            # 2. Convert to Average Power (Watts) using Duty Cycle
            avg_power_tensor = peak_power_tensor * duty_cycle_scaling
            
            active_watts = avg_power_tensor.detach().cpu().numpy().flatten()
            ...
            if frame_nr % 50 == 0:
                # Log in mW (Average Power is usually much lower than Peak)
                print(f"Frame {frame_nr}: Total Avg {total_w*1000:.4f} mW")
    ...
    # A. SAVE .NPY FILE (Average Power per Electrode)
    npy_path = results_dir / 'average_power_per_electrode.npy'
    np.save(npy_path, np.array(full_grid_history) * 1000) # Save in mW
    print(f"[1/3] Saved Raw Data: {npy_path}")

    # B. SAVE TOTAL AVERAGE POWER PLOT
    fig, ax1 = plt.subplots(figsize=(10, 6))
    
    color = 'tab:green' # Green for Average
    ax1.set_xlabel('Time (s)')
    ax1.set_ylabel('Total Average Power (mW)', color=color)
    ax1.plot(time_axis, np.array(total_power_hist) * 1000, color=color, linewidth=1.5)
    ax1.tick_params(axis='y', labelcolor=color)
    ax1.grid(True, alpha=0.3)
    ax1.set_title(f"Total Average Power: {video_filename}\n(Duty Cycle Factor: {duty_cycle_scaling:.4f})")

    stats_path = results_dir / 'total_average_power.png'
    plt.tight_layout()
    plt.savefig(stats_path, dpi=150)
    plt.close()
    print(f"[2/3] Saved Plot: {stats_path}")

    # C. GENERATE VIDEO (avg_power_analysis.avi)
    print("[3/3] Rendering avg_power_analysis.avi...")
    
    playback_cap = cv2.VideoCapture(str(video_path))
    combined_res = (1600, 800)
    video_out_path = results_dir / 'avg_power_analysis.avi'
    ...
    lut = cv2.applyColorMap(np.expand_dims(lut_range, 1), cv2.COLORMAP_VIRIDIS).squeeze() # Viridis for Avg
    ...
    for i in range(len(full_grid_history)):
        ...
        frame_watts = full_grid_history[i] * 1000 
        ...
        stats = f"T={time_axis[i]:.2f}s | Avg Total: {total_power_hist[i]*1000:.2f}mW"
