"""
Render phosphene outputs from images or videos.
...
The default folder layout is:
    results/phosphenes/<input-type>/<media-name>/<electrode-grid>/<view-angle>/<preprocessing-method>/

For video inputs the main output is:
    results/phosphenes/video/<video-name>/<electrode-grid>/<view-angle>/<preprocessing-method>/<video-name>_phosphenes.mp4

By default, video runs also save a side-by-side input/stimulus/phosphene
comparison video for debugging.
"""
...
def resolve_repo_path(path_str: str) -> Path:
    # Allow CLI paths to be given either as absolute paths or relative to the
    # repository root.
    path = Path(path_str)
...
def resolve_coords_yaml(coords_yaml: str | None) -> Path:
    # Prefer the current default dipole grid when the user does not provide an
    # explicit coordinate file.
    if coords_yaml:
...
def build_phosphene_coordinates(params: dict, coords_yaml: Path) -> tuple[utils.Map, utils.Map]:
    ...
    if hasattr(cortex_models, "get_full_field_mapping_from_cortex"):
        # Newer mapping API: returns both percept-space coordinates and the
        # cortical coordinates used for raster grouping.
        ...
        # Backwards-compatible path for older mapping helpers.
        ...
        if isinstance(mapping_out, tuple):
            phosphene_coordinates, remaining_indices = mapping_out
        else:
            phosphene_coordinates = mapping_out
            remaining_indices = np.arange(len(phosphene_coordinates), dtype=np.int64)

        remaining_indices = np.asarray(remaining_indices, dtype=np.int64)
        x_expanded, y_expanded = cortex_models.expand_electrode_coordinate_arrays(x_raw, y_raw, remaining_indices)
        raster_coordinates = utils.Map(
            x=x_expanded[remaining_indices],
            y=y_expanded[remaining_indices],
        )
...
def main() -> None:
    ...
    parser.add_argument("--params", type=str, default="config/params.yaml")
    parser.add_argument("--coords-yaml", "--coords_yaml", dest="coords_yaml", type=str, default="config/coords_400um.yaml")
    parser.add_argument(
        "--render-resolution",
        "--render_resolution",
