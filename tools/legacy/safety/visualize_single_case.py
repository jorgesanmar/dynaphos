"""
Create a compact overview plot for one safety simulation case.

The input can be either a case directory containing safety_metrics.npz or the
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a one-page dashboard for a single safety_metrics.npz case."
    )
    parser.add_argument(
        "--input",
        required=True,
        help="Case directory, safety_metrics.npz file, or a directory containing exactly one case.",
    )
    parser.add_argument(
        "--out",
        default=None,
...
def resolve_npz_path(input_path: str | Path) -> Path:
    ...
    if path.is_file():
        if path.name != "safety_metrics.npz":
            raise ValueError(f"Expected safety_metrics.npz, got: {path}")
        return path

    direct = path / "safety_metrics.npz"
    if direct.exists():
        return direct

    matches = sorted(path.rglob("safety_metrics.npz")) if path.exists() else []
    if len(matches) == 1:
        return matches[0]
    if len(matches) > 1:
        raise ValueError(
            f"Input directory contains {len(matches)} safety_metrics.npz files. "
            "Point --input at one case directory or one NPZ file."
        )
    raise FileNotFoundError(f"No safety_metrics.npz found for: {path}")
...
def get_array(data: dict[str, np.ndarray], key: str) -> np.ndarray | None:
    if key not in data:
        return None
...
def time_axis(
    ...
    *,
    ...
    candidates = ("electrode_time_s", "time_s") if prefer_electrode else ("time_s", "electrode_time_s")
...
def plot_temperature_panel(ax: plt.Axes, data: dict[str, np.ndarray]) -> None:
    ...
    if max_dt is not None:
        _, xlabel = plot_line(ax, data, max_dt, label="max dT", color=ACADEMIC_RED)
        plotted = True
    if mean_dt is not None:
        _, xlabel = plot_line(ax, data, mean_dt, label="mean dT", color=ACADEMIC_BLUE)
        plotted = True
...
def plot_hotspot_area_panel(ax: plt.Axes, data: dict[str, np.ndarray]) -> None:
    series = [
        ("area_gt1_mm2", ">1 deg C", ACADEMIC_BLUE),
        ("area_gt2_mm2", ">2 deg C", ACADEMIC_ORANGE),
        ("area_gt3_mm2", ">3 deg C", ACADEMIC_RED),
    ]
    plotted = False
    xlabel = TIME_AXIS_LABEL
...
def plot_heatmap_panel(ax: plt.Axes, fig: plt.Figure, data: dict[str, np.ndarray]) -> None:
    dT = get_array(data, "dT_final")
    if dT is None:
        add_no_data(ax, "No final heatmap")
        ax.set_title("Final dT Map", fontsize=11)
        return

    heatmap = np.asarray(dT, dtype=np.float64)
...
def summary_text(
    ...
    return (
        ...
        f"peak max dT {format_number(peak_dt, ' deg C')} | "
        f"peak area >1 deg C {format_number(peak_area, ' mm2')} | "
